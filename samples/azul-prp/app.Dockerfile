...
ARG BASE_IMAGE=ghcr.io/holmistr/azul-prime:21
...
CMD [ \
  ...
  rm -f done && \
  java \
    -DSelfDiagnosticRunLevel=1 \
    -DSelfDiagnosticRunPath=. \
    -XX:+UseZingMXBeans \
    ...
    -jar /renaissance-mit-${RENAISSANCE_VERSION}.jar -r ${REPETITIONS} finagle-http && \
